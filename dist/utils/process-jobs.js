...
const processJobs = function (extraJob) {
    ...
    return __awaiter(this, void 0, void 0, function* () {
        debug("starting to process jobs: [%s:%s]", (_b = (_a = extraJob === null || extraJob === void 0 ? void 0 : extraJob.attrs) === null || _a === void 0 ? void 0 : _a.name) !== null && _b !== void 0 ? _b : "unknownName", (_d = (_c = extraJob === null || extraJob === void 0 ? void 0 : extraJob.attrs) === null || _c === void 0 ? void 0 : _c.id) !== null && _d !== void 0 ? _d : "unknownId");
        ...
        if (!extraJob) {
            ...
            debug("job [%s:%s] was passed directly to processJobs(), locking and running immediately", extraJob.attrs.name, extraJob.attrs.id);
        ...
        function jobProcessing() {
            ...
            if (!job.attrs.nextRunAt || job.attrs.nextRunAt <= now) {
                debug("[%s:%s] nextRunAt is in the past, run the job immediately", job.attrs.name, job.attrs.id);
                runOrRetry();
            }
            else {
                // @ts-expect-error linter complains about Date-arithmetic
                const runIn = job.attrs.nextRunAt - now;
                debug("[%s:%s] nextRunAt is in the future, calling setTimeout(%d)", job.attrs.name, job.attrs.id, runIn);
                ...
                if (self._processInterval) {
                    ...
                    if (jobDefinition.concurrency > jobDefinition.running &&
                        ...
                        if (job.attrs.lockedAt && job.attrs.lockedAt < lockDeadline) {
                            debug("[%s:%s] job lock has expired, freeing it up", job.attrs.name, job.attrs.id);
                        ...
                        debug("[%s:%s] concurrency preventing immediate run, pushing job to top of queue", job.attrs.name, job.attrs.id);
                        enqueueJobs(job);
                    }
                }
            }
        ...
        function processJobResult(job) {
            ...
            if (!self._runningJobs.includes(job)) {
                debug("[%s] callback was called, job must have been marked as complete already", job.attrs.id);
