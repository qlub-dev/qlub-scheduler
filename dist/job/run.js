...
const run = function () {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
            ...
            debug("[%s:%s] setting lastRunAt to: %s", this.attrs.name, this.attrs.id, this.attrs.lastRunAt.toISOString());
            ...
            const jobCallback = (error, result) => __awaiter(this, void 0, void 0, function* () {
                ...
                if (error) {
                    ...
                    this.attrs.status = database_1.JobStatus.RUNNING;
                ...
                yield this.save().catch((error) => {
                    debug("[%s:%s] failed to be saved to DB", this.attrs.name, this.attrs.id);
                    reject(error);
                });
                debug("[%s:%s] was saved successfully to DB", this.attrs.name, this.attrs.id);
                if (error) {
                    agenda.emit("fail", error, this);
                    agenda.emit("fail:" + this.attrs.name, error, this);
                    debug("[%s:%s] has failed [%s]", this.attrs.name, this.attrs.id, error.message);
                }
                else {
                    agenda.emit("success", this);
                    agenda.emit("success:" + this.attrs.name, this);
                    debug("[%s:%s] has succeeded", this.attrs.name, this.attrs.id);
                }
                agenda.emit("complete", this);
                agenda.emit("complete:" + this.attrs.name, this);
                debug("[%s:%s] job finished at [%s] and was unlocked", this.attrs.name, this.attrs.id, this.attrs.lastFinishedAt);
            ...
            try {
                ...
                debug("[%s:%s] starting job", this.attrs.name, this.attrs.id);
                if (!definition) {
                    debug("[%s:%s] has no definition, can not run", this.attrs.name, this.attrs.id);
                    throw new Error("Undefined job");
                }
                if (definition.fn.length === 2) {
                    debug("[%s:%s] process function being called", this.attrs.name, this.attrs.id);
                ...
                debug("[%s:%s] unknown error occurred", this.attrs.name, this.attrs.id);
